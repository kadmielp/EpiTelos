...
const getRelativeTime = (timestamp: number, language: 'en' | 'pt-BR') => {
    const now = Date.now();
    ...
    if (minutes < 1) return language === 'pt-BR' ? 'Agora mesmo' : 'Just now';
    if (minutes < 60) return language === 'pt-BR' ? `há ${minutes} min` : `${minutes}m ago`;
    if (hours < 24) return language === 'pt-BR' ? `há ${hours} h` : `${hours}h ago`;
    if (days === 1) return language === 'pt-BR' ? 'Ontem' : 'Yesterday';
...
// Date group helper
const getDateGroup = (timestamp: number, language: 'en' | 'pt-BR'): string => {
    const now = new Date();
    ...
    if (date >= today) return language === 'pt-BR' ? 'Hoje' : 'Today';
    if (date >= yesterday) return language === 'pt-BR' ? 'Ontem' : 'Yesterday';
    if (date >= thisWeek) return language === 'pt-BR' ? 'Esta semana' : 'This Week';
    if (date >= thisMonth) return language === 'pt-BR' ? 'Este mês' : 'This Month';
...
export const ArchiveManager: React.FC<ArchiveManagerProps> = ({ archives, onDelete, onRestore, onInspectContext, functions }) => {
    ...
    const groupedArchives = useMemo(() => {
        ...
        for (const arch of filteredArchives) {
            const group = getDateGroup(arch.timestamp, language);
    ...
    const getFunctionName = (id: string) => {
        return functions.find(f => f.id === id)?.name || t('Unknown Function');
    };
        ...
        <div className="history-page h-full flex flex-col overflow-hidden">
            ...
            <header className="px-8 py-6 border-b border-white/5 flex items-center justify-between bg-neutral-900/40 backdrop-blur-3xl shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-300">{t('WORKSPACE / HISTORY')}</h2>
                        <h1 className="text-xl font-bold text-white tracking-tight">{t('History')}</h1>
                    </div>
                </div>
                ...
                <div className="relative w-72">
                    ...
                    <input
                        type="text"
                        placeholder={t('Search past runs...')}
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="w-full bg-neutral-950/50 border border-white/10 rounded-xl pl-10 pr-4 py-2 text-xs text-white placeholder:text-neutral-400 focus:border-neutral-500/50 outline-none transition-all"
                    />
                </div>
            </header>
            ...
            <div className="flex-1 flex overflow-hidden">
                {/* List Sidebar */}
                <div className="w-80 border-r border-white/5 flex flex-col bg-neutral-950/20 relative">
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-1">
                        {filteredArchives.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center text-neutral-400 p-8 text-center">
                                ...
                                <p className="text-[10px] font-medium leading-relaxed">{t(archives.length === 0 ? 'Your past runs will appear here.' : 'No runs match your search.')}</p>
                            </div>
                        ) : (
                            groupedArchives.map(group => (
                                <div key={group.label} className="mb-2">
                                    <div className="flex items-center gap-3 px-1 py-3 sticky top-0 bg-neutral-950/90 backdrop-blur-sm z-10">
                                        <span className="text-[9px] font-black uppercase tracking-[0.2em] text-neutral-400 whitespace-nowrap">{group.label}</span>
                                        ...
                                        {group.archives.map(arch => (
                                            <button
                                                ...
                                                className={`w-full text-left p-4 rounded-2xl border transition-all relative group ${selectedArchiveId === arch.id
                                                    ? 'bg-neutral-700/10 border-neutral-500/30 '
                                                    : 'bg-white/5 border-transparent hover:border-white/10 hover:bg-white/[0.07]'
                                                    }`}
                                            >
                                                <div className="flex items-center justify-between mb-2">
                                                    <span className="text-[9px] font-black text-neutral-200 uppercase tracking-widest">{getRelativeTime(arch.timestamp, language)}</span>
                                                    <span className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest ${selectedArchiveId === arch.id ? 'bg-neutral-500/20 text-neutral-200' : 'bg-white/5 text-neutral-400'
                                                        }`}>{arch.model.split('/').pop()}</span>
                                                </div>
                                                <h4 className={`text-xs font-bold truncate mb-1 ${selectedArchiveId === arch.id ? 'text-white' : 'text-neutral-200'}`}>{arch.title}</h4>
                                                <div className="flex items-center gap-2">
                                                    <p className="text-[9px] text-neutral-400 font-bold uppercase tracking-tight truncate">{getFunctionName(arch.functionId)}</p>
                                                ...
                                                {selectedArchiveId === arch.id && (
                                                    <div className="absolute right-4 bottom-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <button
                                                            ...
                                                            className="p-2 bg-neutral-500/10 hover:bg-neutral-500/20 text-neutral-200 rounded-xl border border-neutral-500/20 transition-all shadow-xl backdrop-blur-md"
                                                            title={t('Delete entry (Del)')}
                                                        >
                    ...
                    {pendingDeleteId && (
                        <div className="absolute bottom-0 left-0 right-0 p-4 bg-neutral-900/95 backdrop-blur-xl border-t border-neutral-500/20 z-20" style={{ animation: 'slideUp 0.2s ease-out' }}>
                            <p className="text-xs text-neutral-200 font-bold mb-3">{t('Delete this archive permanently?')}</p>
                            <div className="flex gap-2">
                                <button
                                    onClick={handleConfirmDelete}
                                    className="flex-1 px-3 py-2 bg-neutral-500/20 hover:bg-neutral-500/30 text-neutral-200 rounded-xl border border-neutral-500/30 text-[10px] font-black uppercase tracking-widest transition-all"
                                >
                                    {t('Delete')}
                                </button>
                                <button
                                    onClick={() => setPendingDeleteId(null)}
                                    className="flex-1 px-3 py-2 bg-white/5 hover:bg-white/10 text-neutral-300 rounded-xl border border-white/10 text-[10px] font-black uppercase tracking-widest transition-all"
                                >
                                    {t('Cancel (Esc)')}
                                </button>
                            </div>
                ...
                {/* Preview Main Area */}
                <div className="flex-1 overflow-hidden bg-neutral-900/10 relative flex flex-col">
                    {selectedArchive ? (
                        <>
                            <div className="px-8 py-6 border-b border-white/5 flex items-center justify-between shrink-0">
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 px-6 py-3 bg-neutral-700 hover:bg-neutral-500 text-white rounded-xl  transition-all active:scale-[0.98] text-[10px] font-black uppercase tracking-widest"
                                >
                                    <RotateCcwIcon className="w-4 h-4" /> {t('Restore to Laboratory')}
                                </button>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar space-y-8">
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-neutral-200">
                                        <div className="w-1 h-3 bg-neutral-500 rounded-full" /> {t('User Prompt')}
                                    </div>
                                    <button
                                        onClick={() => {
                                        ...
                                        className="text-[9px] font-bold text-neutral-400 hover:text-neutral-200 flex items-center gap-1.5 transition-colors uppercase tracking-widest"
                                    >
                                        <CopyIcon className="w-3 h-3" /> {t(copyStatus ? 'Copied!' : 'Copy Prompt')}
                                    </button>
                                </div>
                                <div className="bg-neutral-950/40 border border-white/5 rounded-3xl p-6 text-neutral-200 text-sm leading-relaxed whitespace-pre-wrap relative overflow-hidden group">
                                    <div className="absolute inset-0 bg-neutral-500/[0.02] opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
                                ...
                                {selectedArchive.contextIds.length > 0 && (
                                    <div className="pt-2 space-y-3">
                                        <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-neutral-400">
                                            <div className="w-1 h-3 bg-neutral-700 rounded-full" /> {t('Sources')} ({selectedArchive.contextIds.length})
                                        </div>
                                        <div className="flex flex-wrap gap-2">
                                            {selectedArchive.contextIds.map((cid, index) => {
                                                ...
                                                return (
                                                    <button
                                                        key={cid}
                                                        title={`${t('Click to inspect: ')}${cleanName}`}
                                                        onClick={() => onInspectContext(cid)}
                                                        className="flex items-center gap-2 px-3 py-1.5 bg-white/[0.04] border border-white/10 rounded-lg text-[10px] text-neutral-200 font-bold group hover:bg-neutral-500/10 hover:border-neutral-500/30 transition-all cursor-pointer"
                                                    >
                                                        <div className="w-1.5 h-1.5 rounded-full bg-neutral-600 group-hover:bg-neutral-500 transition-colors" />
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-neutral-200">
                                        <div className="w-1 h-3 bg-neutral-500 rounded-full" /> {t('Response')}
                                    </div>
                                    <div className="space-y-4">
                                        <div className="bg-white/[0.02] border border-white/5 rounded-3xl p-8 shadow-2xl overflow-hidden relative">
                                            <div className="absolute top-0 left-0 w-2 h-full bg-neutral-700/20" />
                                            <ResponseTerminal
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-neutral-400 p-8 text-center">
                            ...
                            <h3 className="text-sm font-semibold text-neutral-400">{t('Select a run')}</h3>
                            <p className="text-[11px] text-neutral-400 mt-2 max-w-[250px] leading-relaxed">{t('Choose a past run to see its prompt and response.')}</p>
                        </div>
                    )}
                </div>
            </div>
